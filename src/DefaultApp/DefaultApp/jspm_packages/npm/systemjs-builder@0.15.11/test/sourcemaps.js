...
(function(Buffer, process) {
  ...
  var compareSourceMaps = function(filename, expectation, done, transpiler) {
    ...
    instance.build(filename, null, buildOpts).then(function(output) {
  ...
  function writeTestOutput() {
    (new Builder()).loadConfig(configFile).then(function(builder) {
  ...
  function writeSourceMaps(moduleName, transpiler, sourceMapFile) {
    ...
    buildOpts.config = buildOpts.config || {};
    buildOpts.config.transpiler = transpiler || 'traceur';
    instance.build(moduleName, null, buildOpts).then(function(output) {
  ...
  suite('Source Maps', function() {
    test('can render inline', function(done) {
      ...
      var instance = new Builder(configFile);
      instance.build(module, null, {sourceMaps: 'inline'}).then(function(output) {
    ...
    suite('sources paths', function() {
      test('are relative to outFile', function(done) {
        ...
        builder.buildSFX('first', 'dist/output.js', buildOpts).then(function(outputs) {
          var sources = getSources(outputs.sourceMap);
          assert.deepEqual(sources, ['../test/fixtures/test-tree/third.js', '../test/fixtures/test-tree/cjs.js', '../test/fixtures/test-tree/jquery.js', '../test/fixtures/test-tree/some', '../test/fixtures/test-tree/text.txt', '../test/fixtures/test-tree/component.jsx', '../test/fixtures/test-tree/global.js', '../test/fixtures/test-tree/amd.js', '../test/fixtures/test-tree/second.js', '../test/fixtures/test-tree/first.js']);
        }).then(done).catch(err);
      });
      test('are relative to baseURL, if no outFile', function(done) {
        ...
        var opts = {
          sourceMaps: true,
          config: {baseURL: 'test/fixtures/test-tree'}
        };
        builder.buildSFX('first', null, opts).then(function(outputs) {
          var sources = getSources(outputs.sourceMap);
          assert.deepEqual(sources, ['third.js', 'cjs.js', 'jquery.js', 'some', 'text.txt', 'component.jsx', 'global.js', 'amd.js', 'second.js', 'first.js']);
        }).then(done).catch(err);
      });
    });
