...
var toFileURL = require('../lib/utils').toFileURL;
var fs = require('fs');
suite('Test compiler cache', function() {
  ...
  test('Bundle example', function() {
    ...
    return builder.bundle('dynamic-module.js').then(function(output) {
  ...
  test('Bundle example with imported file', function() {
    var builder = new Builder('test/output');
    fs.writeFileSync('./test/output/dynamic-import.js', ['const d = 9;', 'export default d;'].join('\n'));
    fs.writeFileSync('./test/output/dynamic-main.js', ['import d from "./dynamic-import.js";', 'console.log(d);'].join('\n'));
    return builder.bundle('dynamic-main.js').then(function(output) {
      assert(output.source.match(/d = 9/));
      assert(output.source.match(/console/));
      fs.writeFileSync('./test/output/dynamic-import.js', ['import "./dynamic-import2.js";', 'const d = 7;', 'export default d;'].join('\n'));
      builder.invalidate('dynamic-import.js');
      fs.writeFileSync('./test/output/dynamic-import2.js', ['const u = "transitive";', 'export default u;'].join('\n'));
      return builder.bundle('dynamic-main.js');
    }).then(function(output) {
      assert(output.source.match(/transitive/));
      assert(output.source.match(/d = 7/));
      assert(output.source.match(/console/));
      fs.writeFileSync('./test/output/dynamic-import.js', ['const d = 7;', 'export default d;'].join('\n'));
      builder.invalidate('dynamic-import.js');
      return builder.bundle('dynamic-main.js');
    }).then(function(output) {
      assert(!output.source.match(/transitive/));
  ...
  test('Static build example statting check', function() {
    ...
    return builder.buildStatic('static-main.js').then(function() {
  ...
  test('Static build, fetch override', function() {
    ...
    return builder.buildStatic('foo.js', {fetch: function(load, fetch) {
  ...
  test('Static string build', function() {
    var builder = new Builder('test/fixtures/test-tree');
    return builder.bundle('foo.js', {fetch: function(load, fetch) {
        if (load.name.indexOf('foo.js') !== -1) {
          return fs.readFileSync('test/fixtures/test-tree/cjs.js', 'utf8');
        } else {
          return fetch(load);
