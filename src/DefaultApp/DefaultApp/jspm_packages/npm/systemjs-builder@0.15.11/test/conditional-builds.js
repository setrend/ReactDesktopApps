...
suite('Conditional Builds', function() {
  test('Package environment traces all conditional variations', function() {
    return builder.trace('pkg/env-condition').then(function(tree) {
  ...
  test('Conditional interpolation traces all conditional variations', function() {
    return builder.trace('interpolated-#{conditions.js|test}.js').then(function(tree) {
  ...
  test('Boolean conditional', function() {
    return builder.trace('interpolated-1.js#?|browser').then(function(tree) {
  ...
  test('Boolean conditional exclusion', function() {
    return builder.trace('interpolated-1.js#?|browser', {node: true}).then(function(tree) {
      assert.deepEqual(Object.keys(tree), ['interpolated-1.js#?@system-env|browser']);
    });
  });
  test('traceAllConditionals false', function() {
    return builder.trace('pkg/env-condition + interpolated-#{conditions.js|test}.js', {traceAllConditionals: false}).then(function(tree) {
  ...
  test('Browser:false tracing', function() {
    return builder.trace('pkg/env-condition + interpolated-#{conditions.js|test}.js', {browser: false}).then(function(tree) {
  ...
  test('Custom conditions trace', function() {
    return builder.trace('interpolated-#{conditions.js|test}.js', {conditions: {'conditions.js|test': '1'}}).then(function(tree) {
  ...
  test('Environment tracing', function() {
    return builder.trace('pkg/env-condition + interpolated-#{conditions.js|test}.js', {traceConditionsOnly: true}).then(function(tree) {
      assert.deepEqual(Object.keys(tree), ['conditions.js']);
    });
  });
  test('Custom condition build', function() {
    builder.config({map: {'ENV': 'ENV.js'}});
    return builder.trace('custom-conditions.js', {conditions: {
        'ENV|mock': false,
        'ENV|environment': ['dev'],
        'ENV|optimize': true
      }}).then(function(tree) {
  ...
  test('Build including all conditional variations', function() {
    return builder.bundle('pkg/env-condition + interpolated-#{conditions.js|test}.js', 'test/output/conditional-build.js', {sourceMaps: true}).then(function(output) {
