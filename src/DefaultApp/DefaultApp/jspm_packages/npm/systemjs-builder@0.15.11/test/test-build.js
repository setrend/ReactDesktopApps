...
(function(process) {
  ...
  function testPhantom(html) {
    return new Promise(function(resolve, reject) {
      spawn('node_modules/.bin/mocha-phantomjs', [html], {stdio: 'inherit'}).on('close', function(code) {
        if (code !== 0)
  ...
  function doTests(transpiler) {
    ...
    test('Multi-format tree build', function() {
      builder.reset();
      builder.config({transpiler: transpiler});
      return builder.bundle('first.js', 'test/output/tree-build.js', {
        sourceMaps: true,
        minify: minify,
        ...
        var treeFirst;
        Promise.all(['first.js', 'amd.js'].map(builder.trace.bind(builder))).then(function(trees) {
          treeFirst = trees[0];
