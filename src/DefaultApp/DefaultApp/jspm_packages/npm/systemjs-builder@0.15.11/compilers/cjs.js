...
(function(process) {
  ...
  CJSRequireTransformer.prototype.transformCallExpression = function(tree) {
    if (tree.operand.identifierToken && tree.operand.identifierToken.value == this.requireName && tree.args.args.length && tree.args.args.length == 1) {
  ...
  CJSRegisterTransformer.prototype.transformCallExpression = function(tree) {
    if (!this.usesRequireResolve && tree.operand.type == 'MEMBER_EXPRESSION' && tree.operand.operand.identifierToken && tree.operand.operand.identifierToken.value == '$__require' && tree.operand.memberName && tree.operand.memberName.value == 'resolve') {
      this.usesRequireResolve = true;
    }
    return ParseTreeTransformer.prototype.transformCallExpression.call(this, tree);
  };
  CJSRegisterTransformer.prototype.transformMemberExpression = function(tree) {
    if (this.optimize && tree.operand.operand && tree.operand.operand.identifierToken && tree.operand.operand.identifierToken.value == 'process' && tree.operand.memberName == 'env' && tree.memberName.value == 'NODE_ENV') {
  ...
  CJSRegisterTransformer.prototype.transformScript = function(tree) {
    ...
    if (this.usesFilePaths && this.static)
      scriptItemList = parseStatements(["var __filename = '" + this.path + "', __dirname = '" + this.path.split('/').slice(0, -1).join('/') + "';"]).concat(scriptItemList);
    if (this.usesRequireResolve && !this.static) {
      scriptItemList = parseStatements(["$__require.resolve = function(request) { return " + this.systemGlobal + ".get('@@cjs-helpers').requireResolve(request, module.id); };"]).concat(scriptItemList);
    }
    if (this.usesFilePaths && !this.static)
      scriptItemList = parseStatements(["var $__pathVars = " + this.systemGlobal + ".get('@@cjs-helpers').getPathVars(module.id), __filename = $__pathVars.filename, __dirname = $__pathVars.dirname;"]).concat(scriptItemList);
    ...
    scriptItemList = useStrict.concat(parseStatements([(globalExpression ? globalExpression + nl : '') + 'var define, global = this, GLOBAL = this;'])).concat(scriptItemList).concat(parseStatements(['return module.exports;']));
    return new Script(tree.location, parseStatements([this.systemGlobal + '.registerDynamic(' + (this.name ? '"' + this.name + '", ' : '') + JSON.stringify(this.deps) + ', true, function($__require, exports, module) {', '});'], scriptItemList));
  };
  ...
  exports.compile = function(load, opts, loader) {
    ...
    transformer = new CJSRequireTransformer('require', function(dep) {
      return opts.normalize ? load.depMap[dep] : dep;
    }, '$__require');
    tree = transformer.transformAny(tree);
    var deps = opts.normalize ? load.deps.map(function(dep) {
  ...
  function remap(source, map, fileName) {
    ...
    return Promise.resolve({source: output});
  }
  exports.remap = remap;
})(require('process'));
