...
(function(process) {
  ...
  NPMLocation.configure = function(config, ui) {
    ...
    return Promise.resolve().then(function() {
      ...
      if (!doAuth)
        return;
      return auth.configureCredentials(registry, config.authToken && {token: config.authToken} || config.auth && auth.decodeCredentials(config.auth) || npmrcAuth, ui).then(function(_auth) {
        delete config.auth;
        delete config.authToken;
        if (_auth.token)
  ...
  NPMLocation.prototype = {
    lookup: function(repo) {
      ...
      return asp(fs.readFile)(path.resolve(self.tmpDir, repo + '.json')).then(function(lookupJSON) {
        ...
        return asp(request)(auth.injectRequestOptions({
          ...
          headers: lookupCache ? {'if-none-match': lookupCache.eTag} : {}
        }, doAuth && self.auth)).then(function(res) {
          if (res.statusCode == 304)
            return {
              versions: lookupCache.versions,
    ...
    download: function(repo, version, hash, versionData, outDir) {
      ...
      return new Promise(function(resolve, reject) {
        request(auth.injectRequestOptions({
          ...
          strictSSL: self.strictSSL
        }, doAuth && self.auth)).on('response', function(npmRes) {
          ...
          npmRes.pipe(gzip).pipe(tar.Extract({
            path: outDir,
            strip: 1,
            filter: function() {
