...
(function(process) {
  ...
  var systemVersion = require('../package.json!systemjs-json').dependencies.systemjs;
  var tPackages = {
  ...
  exports.dlTranspiler = function(transpilerName, update) {
    return config.load().then(function() {
      ...
      return install.install(installObj, {
        quick: !update,
        dev: true,
        summary: false
      });
  ...
  exports.checkDlLoader = function(transpilerName) {
    return config.load().then(function() {
      ...
      return asp(fs.readdir)(config.pjson.packages).catch(function(err) {
        if (err.code === 'ENOENT')
          return [];
        throw err;
      }).then(function(files) {
  ...
  function dl(name, repo, version) {
    ...
    return endpoint.lookup(pkg.package).then(function(lookup) {
      if (!(nodeSemver.validRange(version)))
        vMatch = version;
      else
        vMatch = Object.keys(lookup.versions).filter(nodeSemver.valid).sort(nodeSemver.compare).reverse().filter(function(v) {
          return nodeSemver.satisfies(v, version);
        })[0];
      vMatchLookup = lookup.versions[vMatch];
      return asp(fs.readFile)(path.resolve(dlDir, '.hash')).then(function(_hash) {
      ...
      if (cached)
        return;
      return endpoint.download(pkg.package, vMatch, vMatchLookup.hash, vMatchLookup.meta, dlDir).then(function() {
  ...
  exports.dlLoader = function(transpilerName, unminified, edge, latest) {
    ...
    return config.load().then(function() {
      ...
      return dl('systemjs', 'github:systemjs/systemjs', !edge ? (!latest ? systemVersion : '^' + systemVersion) : 'master').then(function(version) {
        using.system = version;
        return Promise.all([cp('systemjs/dist/system' + min + '.js', 'system.js'), unminified || cp('systemjs/dist/system.src.js', 'system.src.js'), unminified || cp('systemjs/dist/system.js.map', 'system.js.map'), cp('systemjs/dist/system-csp-production' + min + '.js', 'system-csp-production.js'), unminified || cp('systemjs/dist/system-csp-production.src.js', 'system-csp-production.src.js'), unminified || cp('systemjs/dist/system-csp-production.js.map', 'system-csp-production.js.map'), cp('systemjs/dist/system-polyfills' + min + '.js', 'system-polyfills.js'), unminified || cp('systemjs/dist/system-polyfills.src.js', 'system-polyfills.src.js'), unminified || cp('systemjs/dist/system-polyfills.js.map', 'system-polyfills.js.map')]);
      });
    }).then(function() {
  ...
  exports.init = function init(basePath, ask) {
    ...
    return config.load(ask).then(function() {
      return config.save();
    }).then(function() {
