...
(function(process) {
  ...
  (function() {
    ...
    switch (args[0]) {
      ...
      case 'isntall':
      case 'install':
        options = readOptions(args, ['force', 'link', 'yes', 'lock', 'latest', 'unlink', 'quick', 'dev', 'edge', 'production'], ['override']);
        ...
        if (!depMap && !doUpdate)
          options.lock = true;
        (depMap ? install.install(depMap, options) : install.install(true, options)).then(function() {
      ...
      case 'uninstall':
        ...
        if (options.yes)
          ui.useDefaults();
        install.uninstall(options.args.splice(1)).then(function() {
          ui.log('');
      ...
      case 'resolve':
        ...
        install.resolveOnly(options.only).catch(function(err) {
          if (!err)
      ...
      case 'setmode':
        ...
        if (options.yes)
          ui.useDefaults();
        core.setMode(args.splice(1)).then(function(msg) {
          ui.log('ok', msg);
        }, function(err) {
          ui.log('err', err.stack || err);
        });
      ...
      case 'bundle-sfx':
        var sfxBundle = true;
      case 'bundle':
        options = readOptions(args, ['inject', 'yes', 'skip-source-maps', 'minify', 'no-mangle', 'hires-source-maps', 'no-runtime', 'inline-source-maps', 'source-map-contents'], ['format', 'global-name', 'globals', 'global-defs']);
        if (options.yes)
          ui.useDefaults();
        ...
        if (bArgs.length < 2) {
          (sfxBundle ? bundle.bundleSFX : bundle.bundle)(bArgs[0], undefined, options).catch(function() {
            process.exit(1);
          });
        } else {
          ...
          (sfxBundle ? bundle.bundleSFX : bundle.bundle)(expression, fileName, options).catch(function() {
            process.exit(1);
          });
        }
        break;
      case 'unbundle':
        bundle.unbundle().catch(function(e) {
          ui.log('err', e.stack || e);
          process.exit(1);
        });
        break;
      ...
      case 'compile':
        ...
        build.compileDir(args[1], options).then(function() {
      ...
      case 'registry':
        ...
        if (action === 'config') {
          ...
          return Promise.resolve(registry.configure(args[2])).then(function() {
          ...
          return Promise.resolve(registry.create(args[2], args[3])).then(function(created) {
            if (created)
