...
(function(Buffer, process) {
  ...
  var GithubLocation = function(options, ui) {
    ...
    this.defaultRequestOptions = {strictSSL: 'strictSSL' in options ? options.strictSSL : true};
  ...
  function prepDir(dir) {
    return clearDir(dir).then(function() {
  ...
  function checkStripDir(dir) {
    return asp(fs.readdir)(dir).then(function(files) {
      ...
      return asp(fs.stat)(dirPath).then(function(stat) {
        if (stat.isDirectory())
          return dirPath;
        return dir;
      });
  ...
  GithubLocation.configure = function(config, ui) {
    ...
    return (config.name != 'github' ? Promise.resolve(ui.confirm('Are you setting up a GitHub Enterprise registry?', true)) : Promise.resolve()).then(function(enterprise) {
      if (!enterprise)
        return;
      return Promise.resolve(ui.input('Enter the hostname of your GitHub Enterprise server', config.hostname)).then(function(hostname) {
        config.hostname = hostname;
      });
    }).then(function() {
      return Promise.resolve(ui.confirm('Would you like to set up your GitHub credentials?', true)).then(function(auth) {
        if (auth)
          return configureCredentials(config, ui).then(function(auth) {
            config.auth = auth;
          });
      });
    }).then(function() {
  ...
  GithubLocation.prototype = {
    locate: function(repo) {
      ...
      return new Promise(function(resolve, reject) {
        request(extend({
          ...
          followRedirect: false
        }, self.defaultRequestOptions)).on('response', function(res) {
    ...
    getPackageConfig: function(repo, version, hash, meta) {
      ...
      return asp(request)(extend({
        ...
        qs: {ref: version}
      }, this.defaultRequestOptions)).then(function(res) {
    ...
    download: function(repo, version, hash, meta, outDir) {
      ...
      var remoteString = this.remoteString;
      var self = this;
      return this.checkReleases(repo, version).then(function(release) {
        if (!release)
          return true;
        return new Promise(function(resolve, reject) {
          var inPipe;
          if (release.type == 'tar') {
            (inPipe = zlib.createGunzip()).pipe(tar.Extract({
              path: outDir,
              strip: 0,
              filter: function() {
            ...
            var repoDir;
            inPipe = fs.createWriteStream(tmpFile).on('finish', function() {
              return clearDir(tmpDir).then(function() {
          ...
          }, self.defaultRequestOptions)).on('response', function(archiveRes) {
        ...
        return new Promise(function(resolve, reject) {
          request(extend({
          ...
          }, self.defaultRequestOptions)).on('response', function(pkgRes) {
            ...
            pkgRes.pipe(gzip).pipe(tar.Extract({
              path: outDir,
              strip: 1,
              filter: function() {
                return !this.type.match(/^.*Link$/);
    ...
    build: function(pjson, dir) {
      ...
      return checkMain(main, libDir).then(function(hasMain) {
        if (hasMain)
          return;
        return asp(fs.readFile)(path.resolve(dir, 'bower.json')).then(function(bowerJson) {
          try {
        ...
        }, function() {}).then(function(hasBowerMain) {
          if (!hasBowerMain)
            return;
          pjson.main = main;
        });
